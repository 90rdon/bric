  ...
  if(is_array($all_images)) {
    foreach($all_images as $count => $image) {
      $sizes = unserialize($image->dimensions);
       ...
       if($sizes['width'] >= $glob_slideshow['width']) {
...
/**
  ...
  */
  ...
  //** Get current images */
  $current = get_post_meta($post->ID,'slideshow_images');
  ...
  if ( empty( $gallery_images ) ) {
    ...
    $args = array(
      ...
      'post_status' => null,
      'post_parent' => $post->ID,
      'exclude' => array_merge($exclude, $current[0])
    );
  ...
  if(!$current) {
    ...
    update_post_meta($post->ID, 'slideshow_images',$to_save);
    $current = $to_save;
  }
  ?>
  ...
  */
  function save_in_postmeta(){
    ...
    if( !empty( $_POST['property_slideshow_image_array'] ) ) {
      ...
      update_post_meta($post->ID,'slideshow_images', $image_array);

    } else {

      delete_post_meta( $post->ID, 'slideshow_images' );
    ...
    if( !empty( $_POST['property_gallery_image_array'] ) ) {
      ...
      update_post_meta($post->ID,'gallery_images', $image_array);

    } else {

      delete_post_meta( $post->ID, 'gallery_images' );
...
/**
  ...
  */
  ...
  function wpp_slideshow_get_global_images() {
    ...
    jQuery.post(ajaxurl, {action:"wpp_slideshow_get_global_images", selection: selection, _wpnonce: '<?php echo wp_create_nonce('wpp_get_global_images'); ?>'}, function(result) {
  ...
  */
  function display_slideshow($settings = false) {
    ...
    foreach((array)$images as $image_id){
      ...
      if(!empty($image_object['link'])) {
        ...
        if($settings['link_to_property'] == 'true') {
          ...
          $print_images[] = "<a href='{$parent_link}' class='wpp_global_slideshow_link'>{$this_image}</a>";
        } else {
          $print_images[] = $this_image;
        }
    ...
    if(!empty($print_images)) {
      ...
      $nivoSlider = apply_filters('wpp_slideshow_nivoslider', $nivoSlider);
