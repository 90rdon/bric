...
function properties_del_confirm()
{
...
$jq(document).ready(function(){
	$jq('#merketing_tool_box2').change(function() {
	  // set the window's location property to the value of the option the user has selected
	  //alert($jq(this).val());
	  window.location = $jq(this).val();
	});
...
$jq("ul#menu-footer_menu").append("<?php if($_SESSION['is_login']) { ?><li><a href='/agent-profile'>My Account</a></li><li><a href='/agent-logout'>Logout</a></li><?php } else { ?><li><a href='http://bricrealty.com/agent-login/'>Agent Login</a></li><?php } ?>");
...
var value = document.getElementById('document_type').value;
	if (value=='document')
		  {
			// alert("document");
		    $jq(".video_upload" ).hide();
		    $jq(".document_upload" ).show();
		  }
   if (value=='video')
		  {
			 //alert("video");
		    $jq(".video_upload" ).show();
		    $jq(".document_upload" ).hide()
	      }
...
<script type="text/javascript" src="<?php echo get_template_directory_uri(); ?>/video_popup/jquery.js"></script>
<script type="text/javascript" src="<?php echo get_template_directory_uri(); ?>/video_popup/html5lightbox.js"></script>
