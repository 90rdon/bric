...
/*
Template name: Custom Agent List
...
if(isset($_REQUEST['del_action']) && ($_REQUEST['del_action'] == 'all')){
	...
	$final_id_list = '('.str_replace('-',',',$id_data).')';
	
	
	
	 $query = "DELETE FROM tab_broker WHERE broker_id IN $final_id_list";
	 mysql_query($query);
...
/*$(document).ready(function() {
       $('input[type=checkbox]').on('change', function() {*/
           $.get("agent-list", { 'checkbox[]':$(this).attr('lid') });
      /* }) */
    /*});*/
...
if(count ==1){
	...
	var msg = "Are you sure you want to delete All "+final_count+" Selected Records?";
...
if(confirm(msg)){
	...
	//document.location.href = "/agent-list/?t="+p+"&del_action=all&id_count="+final_count+"&start="+s+"&o_by="+o+"&type="+t;
	document.location.href = "/agent-list/?t="+p+"&del_action=all";
...
{
if(($i == 1) && ($_SESSION['broker_type']=='AD')){
$_SESSION['start'] = $row['broker_id'];
}
?>
