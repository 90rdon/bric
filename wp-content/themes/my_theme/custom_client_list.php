...
Template name: Custom Client List
...
foreach($del_data as $id){
	...
	header("Location:http://bricrealty.com/client-list/");
 ...
 if(isset($_GET['id']) && ($_GET['id'] != '')){
  ...
  $targetpage = '/client-list/'.$url_part;
  ...
  $o_by = 'name';
	...
	$sql = "SELECT * FROM $tbl_name1 ".$ob." LIMIT $start , $limit";
...
function del_client(id,page_no){

if(confirm("Are you sure you want to delete the client")){

	document.location.href = "/my-client-edit/?id="+id+"&a=del&page_no="+page_no+"";
    ...
    $jq(document).ready(function(){
       $jq('#del_all_client').click(function(){
		   ...
		   var num = 0;
			...
			if(confirm(msg)){
			document.location.href = "/client-list/?del_action=all&list="+del_id.substr(1);
			}
        });
    });
...
<a href="javascript:void(0);" id="del_all_client" ><img src="<?php bloginfo('template_directory'); ?>/images/deleate_icon.png" title="delete" height="26px" />
...
<?php } ?>
 ...
 <?php
	...
	<td class="one slct"><div><input class="chcktb4" type="checkbox" id="del_all_<?=$row['client_id']?>" name="del_data" value="<?=$row['client_id']?>"/></div></td>
...
<?php } ?>
   ...
   <td class="one zip"><div><?php echo $row['zip_code']; ?></div></td>
    ...
    <a href="javascript:void(0);" onclick="javascript:del_client('<?=$row['client_id']?>','<?=$page_number?>')"><img src="<?php bloginfo('template_directory'); ?>/images/deleate_icon.png" title="delete" width="20px" /></a></div></td>
