...
class Wolf_Twitter {
	...
	function __construct() {
		...
		define( 'WOLF_TWITTER_URL', get_template_directory_uri().'/library/wolf-twitter');
	...
	function twitter_error( $username, $list = false ) {
		...
		if ( $list ) {
			...
			$output = "<div class=\"container-fluid twitter-container\"><img class=\"twitter-icon\" src=\"".get_template_directory_uri()."/images/twitter-ico.png\" alt=\"Twitter Icon\" /><span class=\"wolf-tweet-text\" style=\"font-size:14px\">$error_message</span></div>";
		...
		if ( $data && is_array( $data ) ) {
			...
			/* Display as single tweet */
				...
				if ( isset( $data[0] ) ) {
					...
					$tweet .= "<div class=\"container-fluid twitter-container\">";
					$tweet .= '<img class="twitter-icon" src="'.get_template_directory_uri().'/images/twitter-ico.png" alt="Twitter Icon" />';
					$tweet .= "<span class=\"twitter-text\">". $this->twitter_to_link( $content )."</span>";
					$tweet .= "</div>";
				} else {
					$tweet = $this->twitter_error( $username, $list );
				}
